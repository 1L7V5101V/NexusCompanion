...
async function loadPlugins() {
  ...
  for (const plugin of window.NexusDashboard._plugins) {
...
async function loadPluginPanel(pluginId) {
  const plugin = window.NexusDashboard._plugins.find((p) => p.id === pluginId);
  if (!plugin) return;
  const ps = state.pluginState[pluginId];
  if (!ps) return;
...
function _formatCell(fmt, value, item, plugin) {
  ...
  const builtinFmt = window.NexusDashboard._formatters[fmtName];
