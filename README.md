...
如果你想让自己的 Nexus 具备和作者差不多的扩展能力，先看社区插件组织：
...
如果 Nexus 已经在运行，你通常可以直接像聊天一样让它安装：

```text
帮我安装这个插件试试看：
https://github.com/nexus-plugins/steam-mcp
```
...
pytest tests/
NEXUS_RUN_SCENARIOS=1 pytest -c pytest-scenarios.ini tests_scenarios/
```

## 工作区

所有运行时数据在 `~/.nexus/workspace/`。
