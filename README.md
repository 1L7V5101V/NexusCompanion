...
调试多模态、Telegram 图片等真实入口时，可以用 Docker 沙盒。它不会挂载宿主机 `HOME`，也不会挂载正式 `~/.nexus/workspace`；所有调试配置、记忆、会话、图片和数据库都只写入 `docker/debug/profiles/<name>/`。
...
pytest tests/           # 单元测试
NEXUS_RUN_SCENARIOS=1 pytest -c pytest-scenarios.ini tests_scenarios/  # 场景测试（真实 LLM）
```
