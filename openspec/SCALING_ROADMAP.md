...
| C1B 迁移工具（M5）                         | 批量 COPY、断点续传、机器可读校验                                                        | C1                        | `phase1b-migration-cutover`（已 archive `2026-08-26-phase1b-migration-cutover`）                     | verified | merge commit `1788de40`；导入/校验基准 `evidence/phase1b/results/`（12,567 行 COPY 10.11s；校验四维全绿）；pytest 1096 passed 无回归（pyright 基线一致）；`openspec/specs/storage-migration` 已 sync |
| C1C 主数据源切换（M6）                       | S0-S4 状态机切 PostgreSQL primary                                              | C1B                       | `phase1b-migration-cutover`（已 archive `2026-08-26-phase1b-migration-cutover`）                     | verified | merge commit `1788de40`；S0→S4 promote/audit/rollback/PITR 演练 `evidence/phase1b/results/`（promote-v1/audit-v1/pitr-v1）；turn control dual-store 定案                        |
...
- **当前阶段**：GOV 治理与文档基线已 verified（merge `e50732d6`）；C1 Storage Foundation 已 verified（merge `0a83314d`）；C0 可观测性与负载工具已 verified（merge `a777ded2`，change `c0-observability-load` 已 archive）；Phase 1B（C1B+C1C）已 verified（merge `1788de40`，change `phase1b-migration-cutover` 已 archive）。
