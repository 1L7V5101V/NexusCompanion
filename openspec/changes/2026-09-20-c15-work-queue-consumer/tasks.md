...
> 对应 `openspec/changes/2026-09-20-c15-work-queue-consumer/`；design.md ADR-1..ADR-8 为实现约束，偏离需先改 design。
...
- **无 `work_attempts` 审计表**：本 change 只用 `attempt_count + last_error`；需要逐次审计时单列扩展（ADR-2）。
