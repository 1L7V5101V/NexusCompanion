"""alembic upgrade/downgrade 封装：避免 env.py 的 fileConfig 破坏调用方日志配置。
...
pytest 进程内执行迁移命令，后续 caplog 断言会捕获不到任何记录。此封装在命令前后快照并
...
def downgrade_to(cfg: Config, revision: str) -> None:
    """downgrade 到指定 revision（C1 测试用于验证新表可安全 drop）。"""
    with _preserved_logging():
