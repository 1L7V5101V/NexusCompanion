...
工作流：
  1. 并行生成 2 条 HyDE 假想记忆条目（"用户在X月重构了nexus…"）
...
class RecallMemoryTool(Tool):
    ...
    description = (
        ...
        "  ✓ 好：'用户重构了 nexus agent 的记忆模块'\n"
        "  ✓ 好：'用户在三月对 nexus 进行了架构重构'\n"
        "  ✗ 差：'我们有做过重构吗'\n"
    ...
    parameters = {
        ...
        "properties": {
            "query": {
                "type": "string",
                "description": (
                    "检索描述，写成陈述句（HyDE 风格）：描述你假设存在的记忆内容。"
                    "例如：'用户重构了 NexusCompanion 的运行时架构'"
                ),
            },
            "memory_type": {
                "type": "string",
...
def _extract_terms(query: str) -> list[str]:
    ...
    策略：
    - ASCII/数字 token（>= 2 chars）直接保留（Phase、nexus、1-4 等）
