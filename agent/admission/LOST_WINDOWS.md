...
| 5 | `core/memory/markdown.py::MarkdownMemoryMaintenance` per-session 维护意图（单槽，至多 1 个 pending） | 丢失 | 维护意图是进程内 deque 槽位 | **可从 durable state 重算**：consolidation/refresh 的触发条件（pending 消息数、`last_consolidated`）持久在 session store，下一个 turn 提交或下轮维护扫描重新触发；意图丢失无数据损失 |
