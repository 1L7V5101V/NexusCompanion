...
python scripts/build_akasha_db.py \
  --config config.toml \
  --sessions-db ~/.nexus/workspace/sessions.db \
  --db-path    ~/.nexus/workspace/memory/akasha.db
```

（不带参数时用默认路径：`~/.nexus/workspace/{sessions.db, memory/akasha.db}`、`config.toml`。）
...
store = AkashaStore("~/.nexus/workspace/memory/akasha.db")

src = sqlite3.connect("~/.nexus/workspace/sessions.db"); src.row_factory = sqlite3.Row
rows = src.execute(
