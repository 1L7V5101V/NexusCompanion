...
class ProactivePromptBuilder:
    ...
    def build_system_prompt(self, plugin_sections: list[str] | None = None) -> str:
        ...
        return (
            ...
            "【你的输出】\n"
            "1. 对每条 Content 给出最终分类：mark_interesting 或 mark_not_interesting。\n"
            "2. 如果最终没有 interesting，调用 finish_turn(decision=skip, reason=no_content)。\n"
            "3. 如果最终有 interesting，生成一条最终消息并按 message_push + finish_turn(decision=reply) 收尾。\n\n"
            "【工具职责】\n"
            "1. Workspace 主动上下文：这是用户当前明确提出并要求你遵守的规则集合。它定义你该怎么筛、哪些要先验证、哪些必须过滤；它不提供新闻事实。\n"
            "2. recall_memory：仅用于 Content 评估——判断单条内容是否可能是用户雷点，或是否可能让用户感兴趣。\n"
