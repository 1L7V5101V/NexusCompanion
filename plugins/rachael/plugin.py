...
class RachaelLastCommandModule:
    slot = "rachael.last_query"
    requires = ("before_turn.acquire_session", "session:session")
    produces = (_CTX_SLOT,)

    def __init__(self, plugin: "RachaelPlugin") -> None:
        self._plugin = plugin
    ...
    async def run(self, frame: Any) -> Any:
        ...
        if command not in {"/akashalast", "/rachael_last"}:
            return frame
...
class RachaelPlugin(Plugin):
    name = "rachael"

    def telegram_bot_commands(self) -> list[tuple[str, str]]:
        if not self.is_active():
            return []
        return [("akashalast", "查看上一轮 Rachael 检索诊断")]
