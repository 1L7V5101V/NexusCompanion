...
from plugins.rachael.core import (
    RachaelNode, ActivationUpdate, EdgeUpdate, ActivationEventRow,
...
class RachaelStore:
    ...
    # 读取单个 turn 节点。
    def get_node(self, key: str) -> RachaelNode | None:
        with self._lock:
            row = self._db.execute(
                "SELECT * FROM akasha_nodes WHERE key = ?",
                (key,),
    ...
    def list_items_for_dashboard(
        ...
        *,
        ...
        # 1. Rachael dashboard MVP 只展示节点状态，不展示原文。
        where = ""
        params: list[object] = []
        if q.strip():
...
# 把 SQLite row 转成 RachaelNode。
def _row_to_node(row: sqlite3.Row) -> RachaelNode | None:
