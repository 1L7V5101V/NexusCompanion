...
class PendingActivation:
    ...
    items: list[RachaelCandidate]
    query_vec: np.ndarray


class RachaelMemoryEngine:
    DESCRIPTOR = MemoryEngineDescriptor(
        ...
        notes={"owner": "plugins.rachael.engine", "truth": "sessions.db/messages"},
    )
    ...
    def __init__(
        ...
        *,
        ...
        self._prev_activation_by_session: dict[str, list[RachaelCandidate]] = {}
        self._graph_lock = threading.RLock()
        self._nodes: dict[str, RachaelNode] = {}
    ...
    def _ensure_idf_table(self) -> None:
        ...
        if stale and self._session_db_path.exists():
            try:
                ...
                print(f"[rachael] built FTS IDF table: {len(idf)} tokens")
            except Exception as exc:  # noqa: BLE001
                print(f"[rachael] IDF build failed, falling back to no-filter: {exc}")
                set_idf_table({})
                return
        idf = load_idf_from_db(conn)
        set_idf_table(idf)

    # 创建 Rachael sidecar 数据库。
    @classmethod
    ...
    def tool_profile(self) -> MemoryToolProfile:
        ...
        return MemoryToolProfile(
            recall=MemoryToolSpec(
                ...
                search_hint="历史对话 原始消息 Rachael 右脑联想",
            ),
            tools=(
                MemoryToolSpec(
                    name="reinforce_memory",
                    description=(
                        "加强当前轮的 Rachael 记忆信号。仅当用户纠正你刚才的回答/工具选择,或明确说以后要记住某条做法时调用。"
    ...
    # 根据 MemoryQuery 执行 Rachael 检索。
    async def query(
        self,
        request: MemoryQuery,
    ) -> MemoryQueryResult:
        # 1. timeline 不是 Rachael MVP 范围，时间线事实仍应走 fetch/search messages。
    ...
    async def ingest(self, request: MemoryIngestRequest) -> MemoryIngestResult:
        # 1. Rachael 的主写入来自 TurnCommitted；ingest 只支持显式 conversation_turn。
        ...
        return MemoryIngestResult(
            accepted=False,
            summary="rachael ingest requires persisted sessions.db messages",
            raw={"reason": "requires_persisted_messages"},
        )

    # Rachael 不通过 memorize/forget 改写事实。
    ...
    def reinforce_items_batch(self, ids: list[str]) -> None:
        # 1. 引用强化暂不改变 Rachael 图，图状态由真实 query 激活驱动。
        return None

    # 撤销由指定消息生成的 Rachael 状态。
    ...
    # 取查询使用的内存图快照。
    def _graph_snapshot(self) -> RachaelActivationSnapshot:
        ...
        with self._graph_lock:
            ...
            return RachaelActivationSnapshot(
    ...
    # 根据 message id 找到本轮 Rachael turn 节点。
    ...
    def _write_query_log(
        ...
        *,
        ...
        seq: int,
        dense_cards: list[RachaelCard],
        ripple_cards: list[RachaelCard],
        text_block: str,
    ) -> None:
        ...
        def _card_to_dict(card: RachaelCard) -> dict[str, object]:
            d: dict[str, object] = {
                "key": card.key,
    ...
    def _format_context_block(
        ...
        *,
        ...
        if dense_cards or ripple_cards:
            ...
            parts.append(f"# Rachael memory now={date_label}")
        if dense_cards:
...
@dataclass(frozen=True)
class _RachaelRetrieval:
    dense_items: list[RachaelCandidate]
    ripple_items: list[RachaelCandidate]
    activation_items: list[RachaelCandidate]
...
def _core_config(config: RachaelConfig) -> CoreConfig:
    return CoreConfig(
...
) -> tuple[list[RachaelCandidate], list[RachaelCandidate], ActivationTrace]:
...
def _compute_candidates_from_snapshot(
    ...
    now_ts: float,
    *,
    snapshot: RachaelActivationSnapshot,
    config: RachaelConfig,
    source_cursor: sqlite3.Cursor | None = None,
    soft_recall: bool = False,
    return_limit: int | None = None,
...
def _card_dedupe_key(card: RachaelCard) -> tuple[str, str]:
...
def _card_ts(card: RachaelCard) -> float:
    try:
...
def _sort_cards_by_time(cards: list[RachaelCard]) -> list[RachaelCard]:
...
# 格式化一个记忆块。
def _format_cards(title: str, cards: list[RachaelCard]) -> str:
...
def _card_to_record(card: RachaelCard, *, injected: bool) -> MemoryRecord:
...
# 把 card 转成 raw item。
def _card_to_raw(card: RachaelCard) -> dict[str, object]:
...
def _source_ref_ids(source_ref: str) -> list[str]:
    # 1. source_ref 始终由 Rachael 生成，解析失败时回退空列表。
    try:
...
def _candidate_signals(item: RachaelCandidate) -> dict[str, object]:
