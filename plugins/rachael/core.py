"""
rachael/core.py — 纯算法核心

Rachael RAR（Ripple Activation & Recall）引擎的纯算法层。
...
def reinforce_boost_from_extra(extra: object) -> float:
    ...
    if not isinstance(parsed, dict):
        return 1.0
    mark = cast("dict[object, object]", parsed).get("rachael_reinforce")
    if not mark:
        return 1.0
...
def load_idf_from_db(conn: sqlite3.Connection) -> dict[str, float]:
    """从 rachael.db 的 fts_token_idf 表加载 IDF 字典。"""
    try:
...
class CoreConfig:
    """算法配置。字段与 RachaelConfig 保持一致的命名和默认值。"""
    dense_top_k: int = 10
...
@dataclass(frozen=True)
class RachaelActivationSnapshot:
...
def activation_edge_updates(
    current_key: str,
    candidates: list[RachaelCandidate],
    ts: float,
...
def reinforced_activation_items(
    current_items: list[RachaelCandidate],
    previous_items: list[RachaelCandidate],
    reinforce_boost: float,
) -> list[RachaelCandidate]:
...
def load_state(
    path: str,
) -> tuple[dict[str, RachaelNode], dict[tuple[str, str], float], dict[str, tuple[int, int]]]:
...
def recover_resource(node: RachaelNode, now_ts: float) -> float:
    """计算短期资源恢复后的值（按真实时间）。"""
    gap = max(0.0, now_ts - node.last_resource_ts)
    return 1.0 - (1.0 - node.resource) * math.exp(-gap / RESOURCE_RECOVER_TAU)


def decayed_strength(node: RachaelNode, now_ts: float) -> float:
    """计算长期强度衰减后的值（按真实时间）。"""
...
def dense_scores(query_vec: np.ndarray, nodes: dict[str, RachaelNode]) -> dict[str, float]:
...
def dense_candidates(
    ...
    *,
    ...
    return [
        RachaelCandidate(key=key, source="Dense", ripple=0.0, direct=score,
...
def score_candidates(
    ...
    *,
...
) -> tuple[list[RachaelCandidate], list[RachaelCandidate]]:
    """计算最终 Ripple 分数，返回 (candidates, suppressed)。"""
    all_candidates: dict[str, RachaelCandidate] = {}
    ...
    for index, key in enumerate(keys):
        ...
        all_candidates[key] = RachaelCandidate(
    ...
    for child in list(all_candidates.values()):
        ...
        all_candidates[bridge.key] = RachaelCandidate(
    ...
    candidates: list[RachaelCandidate] = []
    suppressed: list[RachaelCandidate] = []
...
def graph_expand_candidates(
    ...
    for key, item in aggregate.items():
        ...
        candidates.append(RachaelCandidate(
...
def merge_active_candidates(
    candidates: list[RachaelCandidate],
    graph_candidates: list[RachaelCandidate],
    limit: int,
) -> list[RachaelCandidate]:
    best_by_key: dict[str, RachaelCandidate] = {}
