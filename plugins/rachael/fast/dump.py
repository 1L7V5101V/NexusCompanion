"""dump_to_db —— 把内存 MemoryStore 一次性批量落进已开的 RachaelStore（sqlite），复用 embedding_cache。
...
from plugins.rachael.core import serialize_f32
...
def dump_to_db(store: RachaelStore, mem) -> dict[str, int]:
