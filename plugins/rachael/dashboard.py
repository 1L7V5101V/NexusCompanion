...
def plugin_enabled(app: FastAPI) -> bool:
    return _active_memory_engine(app) == "rachael"


class RachaelInspectorReader:
    def __init__(self, store: RachaelStore) -> None:
        self._store = store
...
def register(app: FastAPI, plugin_dir: Path, workspace: Path) -> list[object]:
    ...
    def get_rachael_graph_query(query_id: str) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=404, detail="Rachael 检索记录不存在") from None
