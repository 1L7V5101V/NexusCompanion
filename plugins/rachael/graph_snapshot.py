...
def default_snapshot_path(workspace: Path) -> Path:
    return workspace / "memory" / "rachael_graph_snapshot.json"
...
def read_graph_signature(rachael_db_path: Path) -> GraphSnapshotSignature:
    with sqlite3.connect(str(rachael_db_path)) as db:
        node_row = db.execute(
...
def build_snapshot_to_file(
    *,
    ...
    payload = build_snapshot(
        rachael_db_path=rachael_db_path,
...
def build_snapshot(
    *,
    ...
    signature = read_graph_signature(rachael_db_path)
    with sqlite3.connect(str(rachael_db_path)) as rachael_db:
        rachael_db.row_factory = sqlite3.Row
        all_edges = _load_merged_edges(rachael_db, min_co_count=config.min_co_count)
        keys = {edge.src for edge in all_edges}
        keys.update(edge.dst for edge in all_edges)
        nodes_by_key = _load_nodes(rachael_db, keys)
