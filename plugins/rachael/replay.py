...
class ReplayStore(Protocol):
    """RachaelReplayRuntime 依赖的 store 接口。

    落库版 store.RachaelStore 与内存版 fast.MemoryStore/CapturingMemoryStore 都结构性满足，
    runtime 不关心底层是 sqlite 还是内存。
    """

    def list_nodes(self) -> list[RachaelNode]: ...
...
class RachaelReplayRuntime:
    def __init__(
        ...
        *,
        ...
        # turn_key -> gain_boost：来自 messages.extra["rachael_reinforce"]，重放时定向加厚该轮边。
    ...
    def _activate_before_turn(
        ...
        if query_text != message.content:
            raise ValueError(f"Rachael replay 缺少 strip 后 query embedding: {message.id}")
...
def _card_to_log_item(card: RachaelCard) -> dict[str, object]:
...
def _turn_messages(
    ...
    *,
    ...
    if parsed is None:
        raise ValueError(f"Rachael turn key 无法解析: {key}")
    ...
    if not has_user:
        raise LookupError(f"Rachael replay 找不到 turn 内容: {key}")
