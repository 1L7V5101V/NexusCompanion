...
class RaceHarness:
    def __init__(
        ...
        *,
        ...
        self.timeout = timeout
        self._tmpdir = TemporaryDirectory(prefix="nexus-race-")
...
def _parse_args() -> argparse.Namespace:
    ...
    _ = parser.add_argument(
        "--scenario",
        default=os.environ.get("NEXUS_RACE_SCENARIO", "all"),
        choices=["all", *SCENARIOS.keys()],
    )
    _ = parser.add_argument(
        "--timeout",
        type=float,
        default=float(os.environ.get("NEXUS_RACE_TIMEOUT", "2")),
    )
    _ = parser.add_argument(
        "--trace",
        type=Path,
        default=(
            Path(os.environ["NEXUS_RACE_TRACE"])
            if os.environ.get("NEXUS_RACE_TRACE")
            else None
        ),
    )
    _ = parser.add_argument(
        ...
        type=Path,
        default=(
            Path(os.environ["NEXUS_RACE_CONFIG"])
            if os.environ.get("NEXUS_RACE_CONFIG")
            else None
        ),
    )
    _ = parser.add_argument(
        ...
        type=Path,
        default=(
            Path(os.environ["NEXUS_RACE_WORKSPACE"])
            if os.environ.get("NEXUS_RACE_WORKSPACE")
            else None
        ),
    )
