...
class ProbePaths:
    ...
    def socket(self) -> Path:
        return self.profile_dir / "nexus.sock"
...
def _run_compose(paths: ProbePaths, args: list[str]) -> None:
    _ = subprocess.run(
        ...
        cwd=paths.repo,
        env={**dict(os.environ), "NEXUS_DEBUG_PROFILE": paths.profile},
        check=True,
    )
...
def _extract_marker(text: str) -> str:
    match = re.search(r"NEXUS_BG_DONE_[0-9a-f]{32}", text)
...
async def _run_probe(args: argparse.Namespace) -> None:
    ...
    try:
        if args.reset_workspace:
            _run_compose(paths, ["run", "--rm", "nexus-debug", "reset-workspace"])
        if args.start_agent:
        ...
        command = (
            ...
            'print("NEXUS_BG_START", flush=True); '
            "time.sleep(18); "
            'print("NEXUS_BG_DONE_"+uuid.uuid4().hex, flush=True)' "'"
        )
        prompt = (
