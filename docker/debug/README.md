...
这个目录用于临时调试真实入口，例如 Telegram 图片、多模态链路、独立 bot 配置。调试容器基于 Arch Linux，沙盒不会挂载宿主机 `HOME`，也不会挂载正式 `~/.nexus/workspace`。
...
```bash
docker compose -f docker/debug/docker-compose.yml run --rm nexus-debug setup
```
...
```bash
docker compose -f docker/debug/docker-compose.yml up nexus-debug
```
...
```bash
NEXUS_DEBUG_PROFILE=multimodal docker compose -f docker/debug/docker-compose.yml run --rm nexus-debug setup
NEXUS_DEBUG_PROFILE=multimodal docker compose -f docker/debug/docker-compose.yml up nexus-debug
```
...
```bash
docker compose -f docker/debug/docker-compose.yml run --rm nexus-debug cli
```

CLI socket 固定为 `/sandbox/nexus.sock`，不会连接正式实例。

## 打开调试 Dashboard

```bash
docker compose -f docker/debug/docker-compose.yml run --rm --service-ports nexus-debug dashboard
```
...
```bash
docker compose -f docker/debug/docker-compose.yml run --rm nexus-debug reset-workspace
```
...
```bash
docker compose -f docker/debug/docker-compose.yml run --rm nexus-debug \
...
```text
NEXUS_RACE_SCENARIO  选择单个场景，默认 all
NEXUS_RACE_TIMEOUT   每个等待点的超时秒数，默认 2
NEXUS_RACE_TRACE     写出 JSON 结果的路径
NEXUS_RACE_CONFIG    指定 config.toml；不指定时生成无外部 channel 的最小配置
NEXUS_RACE_WORKSPACE 指定临时 workspace；不指定时使用临时目录
```
