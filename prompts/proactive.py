...
def build_compose_prompt_messages(
    *,
    ...
    system_msg = (
        ...
        "7. 如果「今天的新内容」为空，或其中没有任何可引用的事实依据，不要把消息写成新闻播报；"
        f" 做不到就输出 {no_content_token}。\n"
        "8. 如果「用户偏好记录」里出现了明确的禁推/过滤/不要推送规则，且该规则与候选内容匹配，必须直接输出"
...
6. 若抓到的内容仍然不足以支撑任何具体提炼，只能基于标题做最小转述，不要扩写。
