...
message 若 should_send=true，写要发给用户的话（口语化，不要像系统通知）。**必须用中文写**，无论引用内容的原始语言是英文还是其他语言，转述和评论都必须用中文表达，不得直接粘贴英文原文。若上一条主动消息已表达过类似的情感关怀，这次消息第一句直接切入新内容，不重复那层铺垫
