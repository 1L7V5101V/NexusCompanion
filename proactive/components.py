...
若 message 引用了 RSS 信息流里的具体内容，请自然点出你是从哪个 RSS 看到的；不需要机械地附完整链接。
对非网页来源（如 novel-kb）不要伪造外链；若系统已有来源信息，会在发送前补充轻量来源提示。
    ...
    async def compose_message(
        ...
        *,
        ...
        system_msg = (
            ...
            "## 性格（与主循环一致）\n"
            f"{PERSONALITY_RULES}\n"
            "如果内容来自 RSS，请自然点出你是从哪个 RSS 看到的，不要机械地堆完整链接；非网页来源不要伪造链接。\n"
            "不要输出JSON，不要解释，不要前缀。"
        )
        user_msg = f"""当前时间：{now_str}
（ISO格式：{now_iso}）
