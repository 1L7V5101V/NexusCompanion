...
function App(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    window.addEventListener("nexus-dashboard-refresh", refresh);
    return () => window.removeEventListener("nexus-dashboard-refresh", refresh);
  ...
  useEffect(() => {
    ...
    window.addEventListener("nexus:goto-session", onGoto);
    return () => window.removeEventListener("nexus:goto-session", onGoto);
  }, []);
...
function formatPluginCell(plugin: PluginConfig, column: DashboardColumn, item: Record<string, unknown>): string {
  ...
  const formatter = plugin.formatters?.[column.fmt || ""] ?? (window as Window & { NexusDashboard?: { _formatters: Record<string, (value: unknown, item?: Record<string, unknown>) => string> } }).NexusDashboard?._formatters[column.fmt || "text"];
