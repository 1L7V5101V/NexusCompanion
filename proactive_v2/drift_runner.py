...
class DriftRunner:
    ...
    def _build_system_prompt(self, skills: list[SkillMeta]) -> str:
        ...
        return (
            ...
            "7. 单次 run 最多只能 send_message 一次。\n"
            "8. send_message 成功后不要再调用 recall_memory / web_fetch / web_search / fetch_messages / search_messages / shell，"
            "后续只允许 write_file、edit_file 和 finish_drift 收尾。\n"
            "9. 执行结束前必须调用 finish_drift 保存状态。\n\n"
            "【可用工具】\n"
